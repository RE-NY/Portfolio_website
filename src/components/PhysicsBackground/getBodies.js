...
function getBody(RAPIER, world) {
    ...
    let x = Math.random() * range - range * 0.5;
    let y = Math.random() * range - range * 0.5 + 3; // Y is offset by +3
    let z = Math.random() * range - range * 0.5; // Z is random
    // physics
  ...
  function getMouseBall(RAPIER, world) {
    ...
    function update(mousePos) {
      // Adjust the Z position to ensure the mouse ball hovers above other balls
      const zPosition = 0.5; // A value slightly above the other balls' expected heights
      mouseRigid.setTranslation({ x: mousePos.x * 2, y: mousePos.y * 2, z: zPosition }); // Reduce scaling factor
